...
public class AStar {
	...
	public AStar(Board b, List<Action> a, Node i, Node g) {
		board = b;
		actionsList = a;
		initialNode = i;
		goalNode = g;
		...
		search();
		System.out.println("AStar Search Completely Done!");
	...
	public void search() {
		do {
			...
			for(int i = 0; i < fringeList.size(); i ++) {
				int dis = fringeList.get(i).distanceCalc(goalNode) + fringeList.get(i).creationCost;
				if(minCost > dis) {
					minIndex = i;
					minCost = dis;
				}

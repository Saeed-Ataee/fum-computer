...
public class GbfSearch {
	...
	public GbfSearch(Board b, List<Action> a, Node i, Node g) {
		board = b;
		actionsList = a;
		initialNode = i;
		goalNode = g;
		...
		search();
		System.out.println("GbfS Search Completely Done!");
	...
	public void search() {
		do {
			...
			for(int i = 0; i < fringeList.size(); i ++) {
				int dis = fringeList.get(i).distanceCalc(goalNode);
				if(minCost > dis) {
					minIndex = i;
					minCost = dis;
				}
